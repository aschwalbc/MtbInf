...
# gamma_runs <- import(here("scripts", "self-clearance", "sc_rates_y35.csv"))
# gamma_runs <- import(here("scripts", "self-clearance", "sc_rates_y50.csv"))
...
isos <- isos[1:3]
runs <- unique(ARI$rep) # 200 repetitions
runs <- runs[1:3]
...
for (c in 1:(length(isos))){
  ...
  for (r in 1:(length(runs))){
    ...
    print(paste0(iso, "_", sprintf("%03d", as.integer(reps))))
    ...
    export(mtb, here("data", "mtb", "isos", paste0(iso, "_", sprintf("%03d", as.integer(reps)), ".Rdata")))
    toc()
  }
}  
