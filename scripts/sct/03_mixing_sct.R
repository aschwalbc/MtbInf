...
WHOinc <- import(here("sources", "who", "WHOinc_2023.csv")) # WHO age and sex specific incidence
...
AGEkey <- data.frame(ageWPP = unique(WPP$ageWPP)) %>% 
  ...
  mutate(ageMIX = case_when(ageWPP %in% c('75-79','80+') ~ '75+', TRUE ~ ageWPP)) %>% 
...
# 5. Age-adjusted ARIs ========== 
input <- c("GP_rev_sct.Rdata", "GPruns_rev_sct.Rdata")
output <- c("mARI_rev_mix_sct.Rdata", "ARI_rev_mix_sct.Rdata")
...
export(ARI_UI, here("data", "ari", "ARI_rev_mix_UI_sct.Rdata"))
