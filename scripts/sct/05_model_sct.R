...
ARI <- import(here("data", "ari", "mARI_rev_mix_pop_sct.Rdata"))
...
for (g in 1:length(gammas)){
  ...
  for (c in 1:(length(isos))){
    ...
    # ARIs at 1950
    ARI0014 <- ari[year == 1961 & ageARI == "00-14", ari][1]
    ARI1544 <- ari[year == 1961 & ageARI == "15-44", ari][1]
    ARI4500 <- ari[year == 1961 & ageARI == "45+", ari][1]
  ...
  export(mtb, here("data", "mtb", sprintf("mMtb_rev_mix_pop_sc%s_sct.Rdata", names(gammas)[g])))
  toc()
}
