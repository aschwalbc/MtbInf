...
# 1. Data ==========
ARI <- as.data.table(import(here("data", "ari", "ARI_rev_sct.Rdata")))
...
for(i in seq(1, length(iso))) {
  ...
  save(erw, file = paste0('data/gp/iso_sct/',cn,'.Rdata'))
  ...
  save(runsdf,file=paste0('data/gp/iso_sct/zz_',cn,'.Rdata'))
...
save(erw_full, file = here("data", "gp", "GP_rev_sct.Rdata"))
save(runsdf_full,file = here("data", "gp", "GPruns_rev_sct.Rdata"))

rm(list = ls())
detach(package:MASS, unload = TRUE) # Detach due to issues with tidyverse "select" 

# 4. GP plots ==========
ARIhist <- as.data.table(import(here("data", "gp", "GP_rev_sct.Rdata")))
ARI <- as.data.table(import(here("data", "ari", "ARI_rev_sct.Rdata")))

iso <- sort(unique(as.character(ARI$iso3)))

ggplot() +
  geom_line(ARIhist, mapping = aes(x = year, y = lari), colour = "#FDB827") +
  geom_ribbon(ARIhist, mapping = aes(x = year, ymin = lower, ymax = upper), fill = "#FDB827", alpha = 0.2) +
  ...
  scale_shape_manual(values = c("prev" = 16, "surv" = 17),
                     labels = c("prev" = "TB prevalence estimates", "surv" = "Immunoreactivity surveys")) +
  coord_cartesian(ylim = c(-10, 0), xlim = c(1950, 2023)) +
  labs(title = 'SCT', x = 'Year', y = 'Annual risk of infection (log)', colour = 'Source', shape = 'Source') +
  theme_bw() + 
  theme(legend.position = 'bottom')

ggplot() +
  geom_line(ARIhist, mapping = aes(x = year, y = exp(lari)), colour = "#FDB827") +
  geom_ribbon(ARIhist, mapping = aes(x = year, ymin = exp(lower), ymax = exp(upper)), fill = "#FDB827", alpha = 0.2) +
  geom_point(ARI, mapping = aes(x = year, y = ari, colour = type, shape = type), show.legend = TRUE) +
  scale_x_continuous(expand = c(0.01, 0.01), breaks = seq(1950, 2025, 10)) +
  scale_y_continuous(labels = scales::label_percent(), breaks = seq(0, 0.005, 0.001)) +
  scale_colour_manual(values = c("prev" = "#CE1126", "surv" = "#003884"),
                      labels = c("prev" = "TB prevalence estimates", "surv" = "Immunoreactivity surveys")) +
  scale_shape_manual(values = c("prev" = 16, "surv" = 17),
                     labels = c("prev" = "TB prevalence estimates", "surv" = "Immunoreactivity surveys")) +
  coord_cartesian(ylim = c(0, 0.005), xlim = c(1950, 2023)) +
  labs(title = 'SCT', x = 'Year', y = 'Annual risk of infection', colour = 'Type', shape = 'Type') +
  theme_bw() + 
  theme(legend.position = 'bottom')
