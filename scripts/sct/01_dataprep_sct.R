...
WHO <- as.data.table(import(here("sources", "who", "WHOest_2000-2023.csv")))
NOT <- as.data.table(import(here("sources", "sct", "PHSnot_2000-2023.csv")))
POP <- as.data.table(import(here("sources", "sct", "ONSpop_2000-2023.csv")))

WHO <- WHO %>% 
  select(iso3, year, starts_with('e_inc_num'), starts_with('c_cdr'), starts_with('e_tbhiv_prct')) %>% 
  rename(inc = e_inc_num, inc_lo = e_inc_num_lo, inc_hi = e_inc_num_hi,
         cdr = c_cdr, cdr_lo = c_cdr_lo, cdr_hi = c_cdr_hi, tbhiv = e_tbhiv_prct, tbhiv_lo = e_tbhiv_prct_lo, tbhiv_hi = e_tbhiv_prct_hi) %>%
  filter(iso3 == 'GBR') %>% 
  left_join(NOT, by = c('iso3', 'year')) %>% 
  mutate(inc = not, inc_lo = not * 0.9, inc_hi = not * 1.1) %>% 
...
KIDinc <- as.data.table(import(here("sources", "who", "WHOinc_2023.csv")))
...
export(ARI, here("data", "ari", "ARI_rev_sct.Rdata"))
