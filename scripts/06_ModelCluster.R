...
#remove loops
#export as a unique combination of focal country and rep

# 1. Load data ==========
ARI <- import(here("data","ari","ARIsubset.Rdata")) 

# Filters
unique(ARI$iso3) # "CHN" "GRC" "IND" "JPN" "PER"
iso <- 'CHN'

unique(ARI$replicate) # 1 2 3 4 5
rep <- 1

ARI <- as.data.table(ARI) %>% filter(iso3 == iso & replicate == rep)
...
export(mtb, here("data", "mtb", "tmp", paste0(iso, "_", rep, ".Rdata")))
